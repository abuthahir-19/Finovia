...
import com.expensetracker.finance.service.CategoryService;
...
public class CategoryController {
    ...
    public CategoryController(CategoryService service, CurrentUserService currentUser) {
        this.service = service;
        this.currentUser = currentUser;
    }

    @GetMapping
    public List<CategoryDto> list(@AuthenticationPrincipal FirebaseUserPrincipal principal) {
        return service.list(currentUser.resolveId(principal));
    }

    @PostMapping
    @ResponseStatus(HttpStatus.CREATED)
    public CategoryDto create(@AuthenticationPrincipal FirebaseUserPrincipal principal,
                              @Valid @RequestBody CategoryRequest body) {
        return service.create(currentUser.resolveId(principal), body);
    }

    @PutMapping("/{id}")
    public CategoryDto update(@AuthenticationPrincipal FirebaseUserPrincipal principal,
